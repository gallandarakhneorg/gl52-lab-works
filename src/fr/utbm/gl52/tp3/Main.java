...
import fr.utbm.gl52.graph.model.arraylist.ArrayListGraph;
import fr.utbm.gl52.graph.model.arraylist.ArrayListGraphEdge;
import fr.utbm.gl52.graph.model.arraylist.ArrayListGraphNode;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		final ArrayListGraph<Integer> graph = new ArrayListGraph<>();

		final GraphListener observer = new GraphListener() {
		...
		final ArrayListGraphNode<Integer> n0 = createNode("n0", graph, random); //$NON-NLS-1$
		final ArrayListGraphNode<Integer> n1 = createNode("n1", graph, random); //$NON-NLS-1$
		final ArrayListGraphNode<Integer> n2 = createNode("n2", graph, random); //$NON-NLS-1$
		final ArrayListGraphNode<Integer> n3 = createNode("n3", graph, random); //$NON-NLS-1$
		final ArrayListGraphNode<Integer> n4 = createNode("n4", graph, random); //$NON-NLS-1$
		final ArrayListGraphNode<Integer> n5 = createNode("n5", graph, random); //$NON-NLS-1$
	...
	private static ArrayListGraphNode<Integer> createNode(String name,
		...
		final ArrayListGraphNode<Integer> node = graph.addNode(name);
		final int nb = random.nextInt(20) + 5;
		for (int i = 0; i < nb; ++i) {
	...
	private static ArrayListGraphEdge<Integer> createEdge(String name,
		...
		final ArrayListGraphEdge<Integer> edge = graph.addEdge(name, a, b);
		final int nb = random.nextInt(20) + 5;
		for (int i = 0; i < nb; ++i) {
